...
class SetupNeonTreeData(metaclass=SingletonMeta):
    ...
    def _load_bounding_boxes(self, label_path, canvas_size):
        ...
        if isinstance(data["annotation"]["object"], list):
            bounding_boxes = [[int(object_dict["bndbox"]["xmin"]), int(object_dict["bndbox"]["ymin"]),
                            int(object_dict["bndbox"]["xmax"]), int(object_dict["bndbox"]["ymax"])] for object_dict in data["annotation"]["object"]]
        elif isinstance(data["annotation"]["object"], dict):
            object_dict = data["annotation"]["object"]
            bounding_boxes = [[int(object_dict["bndbox"]["xmin"]), int(object_dict["bndbox"]["ymin"]),
                            int(object_dict["bndbox"]["xmax"]), int(object_dict["bndbox"]["ymax"])]]
        else:
            LOGGER.warning(f"No bounding boxes found in {label_path.name}.\t{data['annotation']}")
